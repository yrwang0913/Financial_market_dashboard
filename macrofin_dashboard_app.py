...
def chart_recession_periods(fig, recession_periods):
    for start_date, end_date in recession_periods:
        fig.add_vrect(
            x0=start_date, x1=end_date, fillcolor="grey", opacity=0.5, line_width=0
        )
...
def get_commodities_data():
    ...
    for asset_name, asset_ticker in assets.items():
        commodities_data[asset_name] = yf.download(
            asset_ticker, start=start_date, end=end_date
        )["Adj Close"]
...
def get_eurusd_data():
    usd_eur = pd.DataFrame(
        yf.download("EUR=X", start=start_date, end=end_date)["Adj Close"]
    )
...
def get_crypto_data():
    ...
    for crypto_name, crypto_ticker in crypto.items():
        crypto_data[crypto_name] = yf.download(
            crypto_ticker, start=start_date, end=end_date
        )["Adj Close"]
...
def make_treasury_chart():
    ...
    fig_treasury = px.line(
        treasury_yield_data,
        x=treasury_yield_data.index,
        y=["10Y", "2Y"],
        title="10Y 2Y Treasury Yield Spread",
    )
    fig_treasury.add_bar(
        x=treasury_yield_data.index, y=treasury_yield_data["Spread"], name="Spread"
    )
...
def make_ccc_sp500_chart():
    ...
    fig_ccc_sp500.add_trace(
        go.Scatter(
            x=ccc_sp500.index, y=ccc_sp500["CCC-Rated Bond Yield Spread"], name="CCC"
        ),
        secondary_y=False,
    )
...
def make_vix_chart():
    ...
    fig_vix_sp500.add_trace(
        go.Scatter(x=vix_sp500.index, y=vix_sp500["VIX"], name="VIX"), secondary_y=False
    )
...
def display_commodities_chart_mac():
    ...
    with col2_com:
        selected_commodities = st.selectbox(
            "Select the commodities to display:", commodities_data.columns
        )

        recent_commodities_data = (
            commodities_data[[selected_commodities]]
    ...
    with col1_com:
        com_fig = px.line(
            commodities_data,
            x=commodities_data.index,
            y=selected_commodities,
            title=f"{selected_commodities} Prices Over Time",
        )
...
def calculate_portfolio_value_and_return(investment, current_prices):
    investment["current_value"] = investment.apply(
        lambda row: row["amount_invested"]
        * (current_prices[row["asset"]] / row["price_at_investment"]),
        axis=1,
    )
    ...
    return_rate = round(
        ((portfolio_current_value - initial_value) / initial_value * 100), 2
    )
...
def display_main_figures_fin():
    ...
    investment = pd.read_excel(
        "Investment.xlsx", sheet_name="Investment", parse_dates=["date"]
    )
    current_prices = get_current_prices(investment=investment)
    portfolio_value_and_return = calculate_portfolio_value_and_return(
        investment=investment, current_prices=current_prices
    )
    ...
    fin4.metric(
        label="S&P 500",
        value=round(sp500["S&P 500"][-1], 2),
        delta=f"{sp500['Diff (%)'][-1]}" + "%",
    )
    fin5.metric(
        label="USD / EUR",
        value=usdeur["USDEUR"][-1],
        delta=f"{usdeur['Diff (%)'][-1]}" + "%",
    )
    fin6.metric(
        label="TWD / EUR",
        value=twdeur["TWDEUR"][-1],
        delta=f"{twdeur['Diff (%)'][-1]}" + "%",
    )
...
def display_crypto_chart_fin():
    ...
    with col_2_crypto:
        selected_crypto = st.selectbox(
            "Select the crypto to display:", crypto_data.columns
        )

        recent_crypto_data = (
            crypto_data[[selected_crypto]].sort_values("Date", ascending=False).head(8)
        )
...
def macrofin_page_config():
    st.set_page_config(
        ...
        menu_items={"About": "Type the introduction of the page."},
    )
...
def macrofin_page_layout():
    with st.sidebar:
        ...
        page_option = st.selectbox("Sections:", ("Financial Market", "Macroeconomics"))
